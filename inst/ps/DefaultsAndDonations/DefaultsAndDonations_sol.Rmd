...
setwd("C:/Users/Stef/Desktop/MA/Problem Set")
ps.name = "DefaultsAndDonations"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("haven", "dplyr", "ggplot2", "kableExtra", "forcats", "tidyr", "gridExtra", "maxLik", "pracma") # character vector of all packages you load in the problem set
...
#< quiz "Treatment_design"

question: How many different treatments do we have?
sc:
    - 7  
    - 12 *
    - 4
    
success: Great, that's correct!
failure: Try again.
...
Great! You got through the first exercise of this problem set. You are now familiar with the setting of the experiment the authors conducted and are ready to start the analysis. 

If you want to know which awards you have collected so far you can use the command `awards()` in a code chunk. 
...
success: Great, that's correct!
failure: Take a look at the variables and then try again.
...
___(data$___)
#>
sum(data$donated_amount)
#< hint
display("You already used the command we need here in our previous task. The column we need can be found in the list above.")
#>
```
...
The pipe operator (`%>%`) is included in the package `dplyr`. You can use it to perform several steps one after another without creating intermediate results you wouldn't be using afterwards. When you add `%>%` at the end of a line, it can be read as "and then ...".
...
sum_stats <- data %>% 
  group_by(treat) %>%
  summarize(donation_rate = mean(donate)*100,
            av_donation_all = mean(donated_amount),
            av_donation = mean(donated_amount[donate == 1]),
            av_codonrate_all = mean(codonate)*100,
            av_codonrate = mean(codonate[donate == 1])*100,
            av_codon_all = mean(codonated_amount),
...
Great! You managed to solve the second exercise. You learned how to import the given data set and work with it by using commands. Additionally, you got to know the structure of our data set and the variables we have. This will come in handy for the following exercises. 
...
The variables in our data set are defined as follows:
...
data_groups <- data %>%
  filter(donate == 1) %>%
  # dummies for donations
  mutate(group10 = ifelse(donated_amount == 10,1,0),
         group20 = ifelse(donated_amount == 20,1,0),
         group50 = ifelse(donated_amount == 50,1,0),
         groupOther = ifelse(!(donated_amount %in% c(10, 20, 50)),1,0)) %>%
  # dummies for co-donations
  mutate(group0p = ifelse(codonated_pct == 0,1,0),
         group5p = ifelse(codonated_pct == 5,1,0),
...
data_panelA <- data_groups %>%
               group_by(treat) %>%
               summarize(frac10 = sum(group10)/n(), frac20 = sum(group20)/n(), frac50 = sum(group50)/n(), fracOther = sum(groupOther)/n())
...
#< quiz "Effect_of_defaults_on_Donations"

question: Is there an effect on individual donations?
sc:
    - yes *
    - no
    
success: Great, that's correct!
failure: Take a look at the table above and then try again.
...
#>
# Create data frame
data_panelB <- data_groups %>%
               group_by(treat) %>%
               summarize(frac0p = sum(group0p)/n(), frac5p = sum(group5p)/n(), frac10p = sum(group10p)/n(), frac15p = sum(group15p)/n(), 
                         fracOtherp = sum(groupOtherp)/n())
...
#< quiz "Effect_of_defaults_on_Co-Donations"

question: Is there an effect on individual co-donations?
sc:
    - yes *
    - no
...
library(ggplot2)
#< hint
display("In case you don't remember how to import a package, just take a quick look at the very first task of Exercise 2.")
#>
```
...
figure2 <- data %>%
  filter(donate == 1) %>%
  ggplot(aes(x = donated_amount, y = after_stat(density))) +
  geom_histogram(binwidth = 1) +
  facet_wrap(~ default_donation, labeller = labeller(default_donation = levels_labs), scales = "free") +
  coord_cartesian(xlim = c(0, 100), ylim = c(0,0.3)) +
  geom_vline(data = data_lines, aes(xintercept = vline), linetype = 2) +
  scale_x_continuous(breaks = c(0,10,20,50,100)) +
  labs(title = "Donation Distributions by Default Donation Level", 
...
#>
data_action <- data %>%
  mutate(A105 = ifelse(donated_amount == 10 & codonated_pct == 5,1,0),
         A1010 = ifelse(donated_amount == 10 & codonated_pct == 10,1,0),
         A1015 = ifelse(donated_amount == 10 & codonated_pct == 15,1,0),
         A205 = ifelse(donated_amount == 20 & codonated_pct == 5,1,0),
         A2010 = ifelse(donated_amount == 20 & codonated_pct == 10,1,0),
         A2015 = ifelse(donated_amount == 20 & codonated_pct == 15,1,0),
...
data_B4 <- data_action %>%
  group_by(treat) %>%
  summarize(CA105 = sum(A105), CA1010 = sum(A1010), CA1015 = sum(A1015), CA205 = sum(A205), CA2010 = sum(A2010), CA2015 = sum(A2015), CA505 = sum(A505), 
            CA5010 = sum(A5010), CA5015 = sum(A5015))
...
#< award "Drawn to the Default"

Great! You managed to solve the first part of the third exercise. You learned how to create tables and plots (particularly histograms) which lead us to suspect that defaults seem to have quite an impact on individual donor behavior. 
...
data_diff <- data %>% 
  group_by(default_donation) %>%
  mutate(rounded = round(donated_amount)) %>%
  count(rounded) %>%
  mutate(frac = n/sum(n)) %>%
  ungroup() 
```
...
#>
table_diffs <- diffs_donors %>%
  mutate("D10_vs_AD" = (frac_10 - frac_0)*100) %>%
  mutate("D20_vs_AD" = (frac_20 - frac_0)*100) %>%
  mutate("D50_vs_AD" = (frac_50 - frac_0)*100) %>%
  mutate("D20_vs_D10" = (frac_20 - frac_10)*100) %>%
  mutate("D50_vs_D10" = (frac_50 - frac_10)*100) %>%
  mutate("D50_vs_D20" = (frac_50 - frac_20)*100)
```
...
library(gridExtra)
#< hint
display("In case you don't remember how to load a package, just take a quick look at task 3.1.8.")
#>
```
...
# Arrage plots
grid.arrange(plot1, plot2, plot3, blank, plot4, plot5, blank, blank, plot6, ncol = 3, top = "Change in Donation Distribution Due to Defaults")
```
...
#< quiz "Interpretation_of_Figure_4"

question: Take a look at the plots created above. Which of the following statements are correct?
mc:

  - Compared to the active decision environment, there are less people donating 20€ or 50€ when they are in the presence of a default level of 10€.*
  - A higher default level increases the donated amount overall.
  - Defaults induce some donors to donate more than they would normally have.*
  - When raising the default level to 50€, less people opt out.
  - Defaults induce some donors to donate less than they would normally have.*
...
data %>%
  group_by(treat) %>% 
  summarize(mean = mean(donated_amount), sd = sd(donated_amount), n = n()) %>%
  mutate(ic = (sd/sqrt(n)) * 1.96) %>%
...
**Confidence Interval (CI):** "A rule used to construct a random interval so that a certain percentage of all data sets, determined by the confidence level, yields an interval that contains the population value." (Wooldridge, 2020, p.799)

In our case, we can compute a confidence interval for the mean with a known standard deviation using the following formula for the length of the interval: 

$\bar{X} \pm 1.96 \cdot \frac{\sigma}{\sqrt{n}}$ (with $\alpha = 0.05$), where $\bar{X}$ is the sample mean, $\sigma$ the standard deviation and $n$ the sample size. (cf. Hedderich and Sachs, 2018, p.377)
...
#< quiz "Effect_of_defaults_on_Average_Co-Donations"

question: Do defaults affect average co-donations?
sc:
    - yes *
    - no
    
success: Great, that's correct!
failure: Take a look at the plot above and then try again.
...
#< award "Easy by default"

Great! You managed to solve the second part of the third exercise. You got through some more plotting and even got a first impression of how you can check if your results are statistically significant. While doing that, you learned that there is no big effect of defaults on average donations, but for average co-donations.
...
template <- data.frame("Range" = c("\\(f_1\\)", " \\(f_2\\)", "\\(\\hat{F}_1\\)", "\\(\\hat{F}_2\\)", "\\( | \\hat{F}_1 - \\hat{F}_2 | \\)"),
                       "0.0 - 0.9" = c("1", "0", "1 / 10", "0 / 10", "1 / 10"),
                       "1.0 - 1.9" = c("a)", "0", "5 / 10", "0 / 10", "5 / 10"),
                       "2.0 - 2.9" = c("4", "2", "9 / 10", "2 / 10", "c)"),
                       "3.0 - 3.9" = c("1", "6", "10 / 10", "8 / 10", "2 / 10"),
                       "4.0 - 4.9" = c("0", "1", "10 / 10", "b)", "1 / 10"),
                       "5.0 - 5.9" = c("0", "0", "10 / 10", "9 / 10", "1 / 10"),
                       "6.0 - 6.9" = c("0", "0", "10 / 10", "9 / 10", "1 / 10"),
                       "7.0 - 7.9" = c("0", "1", "10 / 10", "10 / 10", "0 / 10"))

template %>%
  kbl(col.names = c("Range", "0.0 - 0.9", "1.0 - 1.9", "2.0 - 2.9", "3.0 - 3.9", "4.0 - 4.9", "5.0 - 5.9", "6.0 - 6.9", "7.0 - 7.9"), 
      align = "c", digits = 4, format = "html") %>%  
...
#< quiz "Filling_the_table" 

parts:

  - question: What's the correct value for a)?
    answer: 4
    success: Great, that's correct!
    failure: That's not correct. How many values do we have within the range of 1.0 - 1.9?
    
  - question: What's the correct value for b)?
    answer: 9/10
    success: Great, that's correct!
    failure: That's not correct. If we had x/10 until this range, and now there are X more values, what does this add up to?
    
  - question: What's the correct value for c)?
    answer: 7/10
    success: Great, that's correct!
    failure: That's not correct. What's the difference between the two values above?
...
success: Great, that's correct!
failure: That's not correct. Take another look at our table and keep in mind what we got for c).
...
In the chunk above, we used `geom_step()` to create a general step function to enhance the understanding of the different components (our sorted data goes on the x-axis, the y-axis should be between 0 and 1). If you want to plot an ecdf in a shorter way, you can use the command `stat_ecdf()` which is also included in the ggplot2 package. 
...
#< preknit
```{r echo=FALSE}
table <- data.frame("\\(n_1 = n_2 =\\)" = c("\\(D_{0.10}\\)", " \\(D_{0.05}\\) ", "\\(D_{0.01}\\)") ,"7" = c(0.571, 0.714, 0.714), "8" = c(0.500, 0.625, 0.750), "10" = c("0.500", "0.600", "0.700"), "11" = c(0.454, 0.545, 0.636))

table %>%
  kbl(col.names = c("\\(n_1 = n_2 =\\)", "7", "8", "10", "11"), align = "c", digits = 4, format = "html", 
      caption = "Values for \\( D_{n_1;n_2;\\alpha} \\)", escape = FALSE, full_width = F) %>%
...
question: Which value from the table above is the one we need?
sc: 
...
success: Great, that's correct!
failure: That's not correct. How many values do we have for each sample? And remember, our significance level is 5%.
...
$p$**-value:** "The smallest significance level at which the null hypothesis can be rejected." (Wooldridge, 2020, p.806)

In our case we have a significance level of $\alpha = 0.05$. This means that if the $p$-value is smaller than $0.05$, we will reject the null hypothesis and accept the alternative hypothesis. 
...
#< award "Certified KS-Tester"

Great! You managed to solve the first part of the fourth exercise. You acquired all the theoretical knowledge on hypothesis testing in general and on the Kolmogoroff-Smirnov Test we need for the next exercise.
...
#< quiz "Comparison_of_treatments"

question: Take a look at the pairs of treatments given below. Which of them are less complicated to be compared to each other?
mc:
...
#>
tests_5p <- data.frame("Treatments" = "p-value", 
                       "(AD,5) vs. (10,5)" = ks.test(data_donation[data_donation$treat=="(AD,5)",]$donated_amount, 
                                                       data_donation[data_donation$treat=="(10,5)",]$donated_amount)$p.value,
                       "(AD,5) vs. (20,5)" = ks.test(data_donation[data_donation$treat=="(AD,5)",]$donated_amount, 
                                                       data_donation[data_donation$treat=="(20,5)",]$donated_amount)$p.value,
                       "(AD,5) vs. (50,5)" = ks.test(data_donation[data_donation$treat=="(AD,5)",]$donated_amount, 
                                                        data_donation[data_donation$treat=="(50,5)",]$donated_amount)$p.value,
                       "(10,5) vs. (20,5)" = ks.test(data_donation[data_donation$treat=="(10,5)",]$donated_amount, 
                                                        data_donation[data_donation$treat=="(20,5)",]$donated_amount)$p.value,
                       "(10,5) vs. (50,5)" = ks.test(data_donation[data_donation$treat=="(10,5)",]$donated_amount, 
                                                        data_donation[data_donation$treat=="(50,5)",]$donated_amount)$p.value,
                       "(20,5) vs. (50,5)" = ks.test(data_donation[data_donation$treat=="(20,5)",]$donated_amount, 
                                                        data_donation[data_donation$treat=="(50,5)",]$donated_amount)$p.value
                       )
```
...
#>
tests_5p %>%
    kbl(col.names = c("Treatments", "(AD,5) vs. (10,5)", "(AD,5) vs. (20,5)", "(AD,5) vs. (50,5)", "(10,5) vs. (20,5)", "(10,5) vs. (50,5)", 
                    "(20,5) vs. (50,5)"), align = "c", digits = 4, format = "html", caption = "Constant default co-donation: 5%", escape = FALSE) %>%
    kable_styling(bootstrap_options = c("striped"))
...
tests_15p %>%
    kbl(col.names = c("Treatments", "(AD,15) vs. (10,15)", "(AD,15) vs. (20,15)", "(AD,15) vs. (50,15)", "(10,15) vs. (20,15)", "(10,15) vs. (50,15)",
                    "(20,15) vs. (50,15)"), align = "c", digits = 4, format = "html", caption = "Constant default co-donation: 15%", escape = FALSE) %>%
    kable_styling(bootstrap_options = c("striped"))
#< hint
display("The code for all three tables is very similar. If you can't figure out what's missing in one blank, maybe you should take a look at the code for a different table and compare them. And don't overlook the last blank inside the caption!")
#>
```
...
#< quiz "Significance_of_p-values"

question: Which of the following statements are true?
mc:

  - None of the treatment comparisons is significant at any level.
  - The comparison of (AD,5) and (10,5) is not statistically significant.*
  - All of the treatment comparisons are significant at the 5% level.
  - The comparison of (AD,10) and (50,10) is statistically significant.*
...
#< award "Top Test Performance!"

Great! You managed to solve the second part of the fourth exercise. You learned how to perform the Kolmogoroff-Smirnov Test in R to verify your assumptions.
...
#< quiz "Donation_utility_function"

question: Now looking at the plot for $\rho = 20$, which of these statements are correct?
mc:

  - The utility of donating x = 20 seems to be the maximum.*
  - Donating more than the generosity level is less useful then donating less.
  - Deviating from choosing x = 20 as your donation decreases the resulting utility.*
...
#< quiz "Donating_the_default"

question: Which of the following expressions represents the utility of donating the default value?
sc:
...
success: Great, that's correct!
failure: The utility in general is defined as $V(x, \rho) = \rho \cdot x - \frac{x^2}{2}$ with $x$ being the donated amount and $\rho$ being the donor generosity. Furthermore we might have the deviation costs $\delta$. Keep this in mind and try again.
...
#< quiz "Donating_preferred_amount"

question: Which of the following represents the utility of donating one's preferred amount?
sc:
...
#< quiz "Formalization_of_case_b"

question: Which of the following represents case b)?
sc:

  - \( V(\rho,\rho) - \delta < V(d, \rho) \textrm{ and } V(\rho,\rho) - \delta \geq - \alpha \)
  - \( V(\rho,\rho) - \delta \geq V(d, \rho) \textrm{ and } V(\rho,\rho) - \delta \geq - \alpha \)*
  - \( V(\rho,\rho) - \delta \geq V(d, \rho) \textrm{ and } V(\rho,\rho) - \delta > - \alpha \)
  - \( V(\rho,\rho) \geq V(d, \rho) \textrm{ and } V(\rho,\rho) - \delta \geq - \delta \)

success: Great, that's correct!
failure: Take a look at the correct answers for the two previous quizzes. Then try again.
...
#< award "As-If Costs Topmodel"

Great! You managed to solve the first part of the fifth exercise. You got through this very theoretical part and are now prepared for the estimation. 
...
#>
data_section4 <- data %>%
  select(default_donation, donated_amount)
```
...
#>
# prepare data set
section4_all <- data_section4 %>% 
  mutate(donated_amount = round(ifelse(donated_amount < 1 & donated_amount > 0, 1, donated_amount))) %>%
  filter(donated_amount <= 300) 

# calculate share of observations used
share_obs <- nrow(section4_all)/nrow(data_section4)
share_obs
```
...
The probability mass function $f(x) = P(X = x_i)$ assigns to each $x \in \mathbb{R}$ the probability with which it occurs according to the discrete random variable $X$.

Each function has to fulfill two requirements: $0 \leq f(x_i) \leq 1$ and $\sum_i f(x_i) = 1$ for $i = 1,2,...$ .

(cf. Auer and Rottmann, 2020, p.193)
...
nzeros_10 <- round(nrow(data_section4[data_section4$default_donation == 10,]) * rate_sample 
                   - nrow(data_section4[data_section4$donated_amount > 0 & data_section4$default_donation == 10,]))

nzeros_20 <- round(nrow(data_section4[data_section4$default_donation == 20,]) * rate_sample 
                   - nrow(data_section4[data_section4$donated_amount > 0 & data_section4$default_donation == 20,]))
...
#>
section4_35 <- data_section4 %>%
  mutate(donated_amount = round(donated_amount)) %>% 
  filter(donated_amount <= 300 & donated_amount > 0)
```
...
section4_35 <- section4_35 %>%
  # add zero contributors for AD
  rbind(., data.frame("default_donation" = rep(0,nzeros_ad), "donated_amount" = rep(0,nzeros_ad))) %>%
  # add zero contributors for 10
  rbind(., data.frame("default_donation" = rep(10,nzeros_10), "donated_amount" = rep(0,nzeros_10))) %>% 
  # add zero contributors for 20
  rbind(., data.frame("default_donation" = rep(20,nzeros_20), "donated_amount" = rep(0,nzeros_20))) %>% 
  # add zero contributors for 50
  rbind(., data.frame("default_donation" = rep(50,nzeros_50), "donated_amount" = rep(0,nzeros_50)))
```
...
#>
data_mle <- section4_all %>%
  filter(default_donation != 0) %>%
  mutate(delta_fun = (donated_amount^2 + default_donation^2)/2 - donated_amount * default_donation)
```
...
# use a subsample
set.seed(42) 
subsample <- sample_n(data_mle, 50000)
...
The command `maxLik()` can handle both equality and inequality constraints. In our case we have an inequality constraint, which has to be of the form $$A \cdot \lambda + B > 0$$

Since we have three variables ( $\lambda_1$, $\lambda_2$ and $\lambda_3$ ) and four inequalities that should be fulfilled, we have to set up a $4 \times 3$ matrix $A$ and a $4 \times 1$ vector $B$. We can define them as follows: 
...
Those are exactly the constraints we want to be considered in our estimation. 
...
#< hint
display("The comments above the blanks already tell you which inequations you have to consider. If you're not sure how to fill in the blanks, just take a look at the last part of the previous task where we defined all the variables you need to solve this task.")
#>
```
...
#< hint
display("In case you don't remember which command you should use here, you can look back on Task 3.2.7 to refresh your memory.")
#>
```
...
#< quiz "Optimal_a_and_b"

question: Looking at the plots, which seem to be the optimal values for $a$ and $b$?
sc:
    - \( a^* = 51.2 \text{ and }b^* = 2.0 \) 
    - \( a^* = 31.3 \text{ and }b^* = 2.0 \) *
    - \( a^* = 31.3 \text{ and }b^* = 2.5 \)
    
success: Great, that's correct!
failure: Try to find the values which yield the highest mean donation. Then try again.
...
#< award "A Personalized Award for You!"

Great! You managed to solve the second part of the fifth and last exercise. You got an insight on how we can use the method of maximum likelihood estimation to derive estimators for our model as well as what potential personalized defaults could look like.  
...
#< award "Congratulations, you made it!"

You worked through this problem set about the effects of defaults on donations. I really hope you had fun while working through the exercises and actually learned something about the theoretical backgrounds. 
